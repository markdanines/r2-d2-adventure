...
class Game {
  ...
  assignRandomCoordinates() {
    const r2d2 = GameUtilities.generateCoordinates();
    const obiWanKenobi = GameUtilities.generateCoordinates();
    ...
    while (r2d2[0] === obiWanKenobi[0] && r2d2[1] === obiWanKenobi[1]) {
      r2d2 = GameUtilities.generateCoordinates();
      obiWanKenobi = GameUtilities.generateCoordinates();
    }
  ...
  assignRandomDirection() {
    const direction = GameUtilities.getRandomNumbersFromRange(4);
    switch (direction) {
      case 0:
  ...
  checkIfWon() {
    if (
      this.r2d2Coords[0] === this.obiWanKenobiCoords[0] &&
      this.r2d2Coords[1] === this.obiWanKenobiCoords[1]
    ) {
  ...
  report() {
    ...
    LoggerService.logInformation(
      this.obiWanKenobiCoords[0],
      this.obiWanKenobiCoords[1]
    );
  }
